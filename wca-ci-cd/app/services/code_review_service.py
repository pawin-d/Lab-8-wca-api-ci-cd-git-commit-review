...
def call_wca_api(content, file_dict=[], url=os.getenv("BASE_URL", DEFAULT_BASE_URL), request_id=str(uuid.uuid4()), apikey=None):
    """Call the Watson Code Assistant API with streaming support and return the complete response text."""
    console = Console()
    ...
    payload = build_prompt_payload(content)
    ...
    with console.status("[bold blue]Processing...", spinner="dots") as status:
        try:
            ...
            for chunk in response.iter_lines():
                if chunk:
                    ...
                    if elapsed > 10 and first_chunk:
                        status.update("[bold yellow]Processing (taking longer than usual)...")
...
def explain(
    ...
    response = call_wca_api(prompt, [source_file.name], apikey=api_key)
